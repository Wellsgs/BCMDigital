<?php		
	...
	if(!verificaSession("cadastro_anuncio.php", $conexao))
	...
	#region efetuando cadastro da anuncios
	if(
		!retornoQueryBolleana(
			"INSERT INTO anuncios (`tipo_id`, `cliente_id`, `segmento_id`,`nm_empresa`, `ds_link`, `ic_home`, `dt_inicio`, `dt_expiracao`) VALUES (".$_POST['tipo_id'].",".$_POST['cliente_id'].",".$_POST['segmento_id'].",'".$_POST['nm_empresa']."','".$_POST['ds_link']."',".$_POST['ic_home'].",'".$_POST['dt_inicio']."','".$_POST['dt_expiracao']."');"
			, $conexao)
	){
		echoJson($flag = 11, false);
	}
	$anuncio_id = mysql_insert_id($conexao);
	#endregion

	#region gravando a imagem do anuncio
	$caminho      = "../datafiles/anuncios/".$anuncio_id."/";
	if(!is_dir($caminho)){
		mkdir($caminho, 0777, true);
	}
	$data         = explode(".", $_FILES['foto_ilustrativa']['name']);
	$extensao     = $data[1];
	move_uploaded_file($_FILES['foto_ilustrativa']['tmp_name'], $caminho.$anuncio_id.".".$extensao);
	#endregion

	LogSistema(
		$_SERVER ['REQUEST_URI'], 
		"Está pagina faz o cadastro de um novo anuncio", 
