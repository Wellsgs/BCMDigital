<?php
	...
	$maximo = retornoQueryUmaField("SELECT MAX(nivel) AS maior FROM cargo_funcionario", $conexao, "maior");

	#region efetuando cadastro de fornecedor
	if(!retornoQueryBolleana("
		UPDATE cargo_funcionario SET nivel =
			CASE
				WHEN nivel = ".$_POST['id']." AND nivel <  ".$maximo."
					THEN
						nivel+1
	...
	LogSistema(
			$_SERVER ['REQUEST_URI'], 
			"Está pagina diminui o nivel de um cargo", 
