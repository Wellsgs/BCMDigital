<?php 
    ...
    LogSistema(
            $_SERVER ['REQUEST_URI'], 
            "Nestá pagina é possível fazer a atualização de um tipo", 
            "",
            $conexao
        );
    ...
    if($flag){
        $resp = mysql_query("SELECT * FROM tipos WHERE id=".$_GET['id']." LIMIT 1;", $conexao);
                  ...
                  <div class="col-md-12">
                        <div class="panel panel-primary">
                            <div class="panel-heading">
                                <h4>Alterar dados do tipo: <?php echo strtoupper($linha['nm_tipo'])?></h4>                            
                            </div>
                            <div class="panel-body collapse in">
                                <div id="example_wrapper" class="dataTables_wrapper" role="grid">
                                    <form action="#" id="atualizar-tipo" name="atualizar-tipo" method="post" class="form-horizontal">
                                        <div class="col-md-12">
                                            <label>Nome do tipo:</label>
                                            <input value="<?php echo strtolower($linha['nm_tipo'])?>" type="text" name="nm_tipo" placeholder="Nome do tipo" class="form-control required" />
                                        </div>
...
$('#atualizar-tipo').validate();

$("#atualizar-tipo button").click(function(){                                                 
    
    var isvalidate=$("#atualizar-tipo").valid();
    if(isvalidate){
        $.ajax(
            {               
                type:"POST",
                url:"../controller/atualizar-tipo.php",
                data:$("#atualizar-tipo").serialize(),
                success : function(data){
                    if(data == "0")
                        alertaSucesso('Alterado com sucesso!', 'Sucesso', '../view/visualizar_tipos.php');
                    else if (data == "1")
