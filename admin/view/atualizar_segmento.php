<?php 
    ...
    LogSistema(
            $_SERVER ['REQUEST_URI'], 
            "Nestá pagina é possível fazer a atualização de um segmento", 
            "",
            $conexao
        );
    ...
    if($flag){
        $resp = mysql_query("SELECT * FROM segmentos WHERE id=".$_GET['id']." LIMIT 1;", $conexao);
                  ...
                  <div class="col-md-12">
                        <div class="panel panel-primary">
                            <div class="panel-heading">
                                <h4>Alterar dados do segmento: <?php echo strtoupper($linha['nm_segmento'])?></h4>                            
                            </div>
                            <div class="panel-body collapse in">
                                <div id="example_wrapper" class="dataTables_wrapper" role="grid">
                                    <form action="#" id="atualizar-segmento" name="atualizar-segmento" method="post" class="form-horizontal">
                                        <div class="col-md-12">
                                            <label>Nome do segmento:</label>
                                            <input value="<?php echo strtolower($linha['nm_segmento'])?>" type="text" name="nm_segmento" placeholder="Nome do segmento" class="form-control required" />
                                        </div>

                                        <div class="col-md-12">
                                            <label>Nome do css:</label>
                                            <input value="<?php echo strtolower($linha['nm_css'])?>" type="text" name="nm_css" placeholder="Nome do css" class="form-control required" />
                                        </div>
...
$('#atualizar-segmento').validate();

$("#atualizar-segmento button").click(function(){                                                 
    
    var isvalidate=$("#atualizar-segmento").valid();
    if(isvalidate){
        $.ajax(
            {               
                type:"POST",
                url:"../controller/atualizar-segmento.php",
                data:$("#atualizar-segmento").serialize(),
                success : function(data){
                    if(data == "0")
                        alertaSucesso('Alterado com sucesso!', 'Sucesso', '../view/visualizar_segmentos.php');
                    else if (data == "1")
